...
Základní stavební jednotka neuronových sítí je umělý neuron (\textit{artificial neuron}) (viz obrázek \ref{fig:neuron}). Tento model je založen na principu reálných neuronů, které se nacházejí v~organizmech. Umělý neuron obsahuje libovolně mnoho vstupních propojení, přes které se mu předávají data v~podobě vstupního vektoru $\textbf{\overrightarrow{x}} \in \mathbb{R}^N$, kde $\textbf{\overrightarrow{x}} = [x_1, x_2, \dots, x_n], n \in N$. Sám neuron obsahuje hodnotu bias $b \in \mathbb{R}$ a vektor vah $\textbf{\overrightarrow{w}} \in \mathbb{R}^N$, kde $\textbf{\overrightarrow{w}} = [w_1, w_2, \dots, w_n], n \in N$, jenž je upravován během trénování neuronu.
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[scale=1.0]{obrazky-figures/TODO.png}
...
% ==Dva jednotlive obrazky vedle sebe==
%\begin{figure}
%    \centering
%    \begin{minipage}{0.7\textwidth}
%        \centering
%        \includegraphics[width=0.3\textwidth]{obrazky-figures/convmapping.png}
...
%    \begin{minipage}{0.45\textwidth}
%        \centering
%        \includegraphics[width=0.4\textwidth]{obrazky-figures/features.png}
...
Každý segment směsi o~délce $L$, $\textbf{x_k} \in \mathbb{R}^{1 \times L}$, kde $k = 1, \dots, \frac{T}{L}$, je transformován na nezápornou reprezentaci $\textbf{w} \in \mathbb{R}^{1 \times N}$ pomocí 1--D konvoluční operace jako
\begin{equation}
  	\textbf{w} = ReLU(x \circledast \textbf{U})
\end{equation}
kde $\textbf{U} \in \mathbb{R}^{N \times L}$ obsahuje $N$ vektorů, každý délky $L$, které reprezentují bázové funkce enkodéru. Operace $\circledast$ značí konvoluční operaci. ReLU je nelineární aktivační funkce, která byla blíže popsána v~podkapitole~\ref{relu}.


\subsection{Dekódování extrahovaných mluvčích}
Pro převod z~reprezentace zpět do podoby audio nahrávky slouží lineární dekodér. Pomocí 1--D dekonvoluční operace rekonstruuje původní signál $\textbf{x}$ jako $\textbf{x} \in \mathbb{R}^{1 \times L}$. Tuto operaci lze definovat jako 
\begin{equation}
\label{equ:decoder}
  \textbf{\hat{x}} = \textbf{w}\textbf{V}
\end{equation}
kde každý řádek v~matici $\textbf{V} \in \mathbb{R}^{N \times L}$ představuje jednu bázovou funkci dekodéru s~délkou $L$.

%-------------------------------

\section{Separační modul}
Cílem separačního modulu je najít váhovanou funkci, neboli masku, pro každého zdrojového mluvčího pro každý výstup enkodéru v~každém časovém kroku. Formálně lze zapsat, že cílem je odhadnout $C$ masek $\textbf{m}_i \in \mathbb{R}^{1 \times N}, i = 1, \dots, C$, kde $C$ představuje počet mluvčích ve směsi. Vektory masek $\textbf{m}_i$ mají takové omezení, že $\sum_{i=0}^{C}\textbf{m}_i=1$, kde $1$ je jednotkový vektor v~$\mathbb{R}^{1 \times N}$. Toto omezení garantuje, že rekonstruované zdroje po sečtení zformují původní směs $\textbf{\hat{x}} = \sum_{i=1}^{C}\textbf{\hat{s}}_i$.
...
\begin{equation}
	\textbf{d_i} = \textbf{w} \odot \textbf{m}_i
\end{equation}
kde $\textbf{d_i} \in \mathbb{R}^{1 \times N}$ je reprezentace každého ze zdrojů a operace $\odot$ je součin po složkách. 

Nakonec jsou reprezentace $\textbf{d_i}$ rekonstruovány zpět na korespondující nahrávky zdrojů $\textbf{\hat{s}}_i, i = 1, \dots, C$ pomocí dekodéru, jak je znázorněno v~\ref{equ:decoder}.
...
Jádrem separační části je \textit{temporal convolutional network} skládající se z~naskládaných  1--D konvolučních bloků s~časovou dilatací jak lze vidět na obrázku \ref{fig:tasnet-stacked-resblocks}. Každá vrstva v~TCN obsahuje $X$ konvolučních bloků se zvyšující se časovou dilatací, která se zvyšuje exponenciálně podle počtu bloků a nabývá hodnot $1, 2, 4, \dots, 2^{X-1}$. Taková sekvence bloků je opakována $R$--krát. Exponenciální růst dilatačního faktoru zajištujě dostatečně velké okno pro využití výhod dlouhých časových závislostí v~signálu řeči. Obrázek \ref{fig:causal_dilation} zobrazuje rostoucí časovou dilataci na vstupních vzorcích signálu.
...
\begin{equation}
	D\texttt{-}conv(\textbf{Y}, \textbf{K}) = concat(\textbf{y}_j \circledast \textbf{k}_j), j = 1, \dots, N
\end{equation}

\begin{equation}
	S\texttt{-}conv(\textbf{Y}, \textbf{K}, \textbf{L}) = D\texttt{-}conv(\textbf{Y}, \textbf{K}) \circledast \textbf{L}
\end{equation}

kde $\textbf{Y} \in \mathbb{R}^{G \times M}$ je vstup do S--conv, $\textbf{K} \in \mathbb{R}^{G \times P}$ je konvoluční jádro o~velikosti $P$, dále $\textbf{y}_j \in \mathbb{R}^{1 \times M}$ a $\textbf{k}_j \in \mathbb{R}^{1 \times P}$ jsou řádky matic $\textbf{Y}$ a $\textbf{K}$. $\textbf{L} \in \mathbb{R}^{G \times H \times 1}$ je konvoluční jádro o~velikosti 1. Operace $1 \times 1$--conv se chová jako plně propojená vrstva a transformuje příznaky do potřebných rozměrů.
...
\section{Normalizace}
Při trénování neuronových sítí může docházet k~fenoménu \todo{uvozovky} vnitřní kovariantní posuv (\textit{internal covariate shift}), kvůli kterému je nutné pečlivě inicializovat parametry a volit menší hodnotu learning rate, což zpomaluje trénování sítě. K~adresování problému slouží zakomponování normalizace do architektury neuronové sítě. Normalizace je aplikována na každý mini--batch trénovacích dat~\cite{ioffe2015batch},~\cite{ba2016layer}.

Konvoluční bloky obsahují normalizační vrstvy, které můžou významně ovlivnit výkon sítě. Ve studii bylo experimentováno s~channel--wise layer normalization (cLN)~\todo{cite}, global layer normalization (gLN) a batch normalization (BN)~\cite{ioffe2015batch}, kterou jsem se rozhodl použít v mé implementaci.
