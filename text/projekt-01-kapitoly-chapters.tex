...
\SetNoFillComment
 ...
 instanciace třídy \texttt{SegmentDataset} a dataloaderu pro trénovací data\;
 instanciace třídy \texttt{SegmentDataset} a dataloaderu pro validační data\;
 
 \For{epocha v~args.Epochs}{
  \tcc{\textit{trénování}}
  loss = 0\;
  \For{směs\_mluvčích z~trénovacího datasetu}{
    \tcc{\textit{cyklus načítá z~dataloaderu segmenty nahrávek, které jsou generovány v~třídě SegmentDataset}}
  	odhadovaná\_separace = \texttt{TasNet}(směs\_mluvčích)\;
...
Po dostatečném natrénování probíhá vyhodnocení sítě. Výsledky trénování jsou měřeny metrikami, které se volí na základě problému, který se síť učila řešit. Výsledky testování sítě jsou následně porovnávány v~závislosti na velikosti daného modelu. Velikost modelu je ovlivněna hyper-parametry X, který ovlivňuje počet konvolučních bloků, hyper-parametrem R, který ovlivňuje počet opakování sekvence bloků a velikost segmentace nahrávek při trénování. Jak hodnoty X a R ovlivňují velikost sítě je dobře vidět na obrázku~\ref{fig:tasnet-stacked-resblocks}.
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.23]{obrazky-figures/s1.png}
    \caption{\label{fig:ref-s1}První mluvčí ze směsi}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.23]{obrazky-figures/s2.png}
    \caption{\label{fig:ref-s2}Druhý mluvčí ze směsi}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.23]{obrazky-figures/mix.png}
...
8 & 2 & 32000 & \textbf{12.71} & \textbf{0.94} & \textbf{2.71} \\ \hline \hline
...
Práce by mohla být rozšířena o~variabilnější dataset pro trénování modelů. Zásadní problém při použití mých naučených modelů v~reálném scénáři je ten, že trénovací dataset není variabilní, když nepočítám různé kombinace konkrétních mluvčích a jejich pohlaví. V~případě, že by nahrávky řečníků byly pořízeny ve variabilnějším prostředí s~rozdílnou technikou, mikrofony a různým ruchem v~pozadí (další hlasy, hudba či tleskot), model by tak byl lépe využitelný. Trénování by také mohlo být rozšířeno o~nahrávky s~větším počtem mluvčích, což by ale vyžadovalo rozsáhlejší implementační úpravy. 
...
Z~vnějšího pohledu by tedy bylo zajímavé rozšířit či navázat na model či vylepšit jeho trénování následovně.
\begin{itemize}
  \item \textbf{Variabilnější dataset} by mohl obsahovat nahrávky zachycené různými zařízeními a z~různého prostředí.
  \item \textbf{Natrénovat větší modely} než jsem trénoval já v~rámci mých experimentů. Bylo by zajímavé vyzkoušet model o~velikosti třeba $X=10$ a sledovat chování a výsledky sítě. To jsem ale z~důvodu časové náročnosti trénování nestihl. 
  \item \textbf{Více zdrojů na nahrávce} jako další mluvčí, hudba a podobně, což by ale obnášelo mnohé implementační změny.
  \item \textbf{Identifikace mluvčího} na separované nahrávce a následné využití například k~ovládání domácího asistenta.
  \item \textbf{Přepis řeči na text}, což vyžaduje separované nahrávky. Systém by tak mohl v~prostředí s~mnoha mluvčími separovat jejich řeč a automaticky tak přepisovat záznam ze schůze. 
\end{itemize}

Z~vnitřního pohledu na síť by mohl být model vylepšen mnoha způsoby, ale osobně bych, v~případě více času, navrhnul a vyzkoušel následující modifikace.
\begin{itemize}
  \item \textbf{Dvě metriky} jako objektivní funkce, místo dosavadní jedné.
  \item \textbf{Dropout vrstvy} zakomponovat do modelu sítě. Bývají aplikovány po konvolučních vrstvách a mohlo by být zajímavé sledovat následné chování.
  \item \textbf{Kauzální konfigurace} sítě a vyzkoušet jiné normalizační techniky.
\end{itemize}
