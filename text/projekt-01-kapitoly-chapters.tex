...
\begin{figure}
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.7\textwidth]{obrazky-figures/ReLU.png} % first figure itself
        \caption{\label{fig:relu}Graf aktivační funkce ReLU}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth]{obrazky-figures/prelu.jpg} % second figure itself
        \caption{\label{fig:prelu}Graf aktivační funkce PReLU}
    \end{minipage}
\end{figure}
...
\begin{equation}
  	w = ReLU(x \circledast U)
\end{equation}
kde $U \in \mathbb{R}^{N \times L}$ obsahuje $N$ vektorů, každý délky $L$, které reprezentují bázové funkce enkodéru. Operace $\circledast$ značí konvoluční operaci. ReLU je nelineární aktivační funkce, která byla blíže popsána v~podkapitole \ref{relu}.
...
\begin{equation}
	d_i = w \odot m_i
\end{equation}
kde $d_i \in \mathbb{R}^{1 \times N}$ je reprezentace každého ze zdrojů a operace $\odot$ je součin po složkách. 

Nakonec jsou reprezentace $d_i$ rekonstruovány zpět na korespondující nahrávky zdrojů $\hat{s}_i, i = 1, \dots, C$ pomocí dekodéru, jak je znázorněno v~\ref{equ:decoder}.
...
\begin{equation}
	D\texttt{-}conv(Y, K) = concat(y_j \circledast k_j), j = 1, \dots, N
\end{equation}

\begin{equation}
	S\texttt{-}conv(Y, K, L) = D\texttt{-}conv(Y, K) \circledast L
\end{equation}
...
\begin{figure}
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.4\textwidth]{obrazky-figures/prelu.jpg} % first figure itself
        \caption{\label{fig:img1}first figure}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.4\textwidth]{obrazky-figures/prelu.jpg} % second figure itself
        \caption{\label{fig:img2}second figure}
    \end{minipage}
\end{figure}
