...
\label{implementace}
\todo{POZN: NN neni tak moc o implementaci, ale ze shrnout zajimave casti a s cim byl problem. impl: co treba jsem resil a co delalo problemy, a pro nekoho kdo to bude pokracovat tak aby treba vedel jak se to vyresi. Tato kapitola NEBUDE DLOUHA (tak kolem 5 stran)}
...
zpracovani datasetu, jedna segmentuje, druha ne..
...
\todo{zminit stride, padding, a mozna obrazek co to je.}
...
\todo{Pseudo algoritmus programu}
\item 
spusteni a predani parametru a nastaveni velikosti site.
\item
nacteni datasetu
...
Pro trénování byly nahrávky, v závislosti na hodnotě hyperparametru, rozděleny na segmenty o délce 4, případně 2 sekund.
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.30]{obrazky-figures/segmentation.jpg}
    \caption{\label{fig:segmentace}Způsob segmentace nahrávek}
\end{figure}

Algoritmus \ref{alg:segmentace} představuje proces, jakým jsou nahrávky segmentovány, jak zobrazuje obrázek \ref{fig:segmentace}.
Funkce $segmentGenerator()$, která slouží jako generátor, pracuje současně se třemi nahrávkami -- se směsí dvou mluvčích, s nahrávkou prvního mluvčího a s nahrávkou druhého mluvčího, které jsou do atributů $self.current_mixture$, $self.current_source1$ a $self.current_source2$ třídy $SegmentDataset$ nahrávány funkcí $loadNextAudio()$ po zpracování předchozí trojice nahrávek.
...
\begin{algorithm}[H]
 ...
 \caption{Algoritmus segmenace nahrávek používá python generátor, který po vrácení hodnoty příkazem yield neztrácí svůj vnitřní stav a po jeho zavolání pokračuje tam, kde skončil}
\end{algorithm}


%\begin{algorithm}[H]
% \KwData{$self.current_mixture$}
% \KwData{$self.current_source1$}
% \KwData{$self.current_source2$}
% \KwData{$segptr = 0$}
% \KwResult{$segmentovaná data o délce SEGMENT_LENGTH$}
% $mix_segment = []$\;
% $s1_segment = []$\;
% $s2_segment = []$\;
% \While{$je dostupná další trojice nahrávek$}{
%  %read current\;
%  \eIf{$self.current_mixture_len < SEGMENT_LENGTH$}{
...
%   }{
%	 \eIf{$segptr + self.SEGMENT_LEN < self.current_mixture_len$}{
%       $mix_segment = self.current_mixture[segptr:(segptr+self.SEGMENT_LEN)]$\;
% 	   $s1_segment = self.current_source1[(segptr+self.SEGMENT_LEN)]$\;
% 	   $s2_segment = self.current_source2[(segptr+self.SEGMENT_LEN)]$\;
% 	   $segptr += self.SEGMENT_LENGTH$\;
...
\section{Trénování}
\todo{Úvod - Pozn: colab + stroj, konkretni hyperparams, vykon a cas trenovani, seg--len}.
