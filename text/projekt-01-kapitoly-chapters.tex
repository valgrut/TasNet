...
%\begin{figure}[H]
%    \centering
%    \includegraphics[scale=0.35]{obrazky-figures/cnn.png}
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[scale=0.5]{obrazky-figures/stride1.png}
...
%\begin{figure}[H]
%    \centering
%    \includegraphics[scale=0.5]{obrazky-figures/stride2.png}
...
\todo{Pridat Grafy trenovani. Popis: Dát validni grafy a udelat druhy graf kde zkombinovat validacni a trenovaci loss do jednoho. Vydelit hlavne hodnoty na ose x poctem segmentu v~jedne epose a ose y vydelit velikosti minibatche a poctem mluvcich ve smesi (2)}.
...
%\begin{table}[hbt]
%\centering
%\caption{Hodnoty hyperparametrů trénovaných sítí}
%\label{hodnoty}
...
%\begin{table}[hbt]
%\centering
%\caption{Hodnoty hyper--parametrů trénovaných modelů}
%\label{tab:trenovani}
%\begin{tabular}{|l|l|l|}
%\hline
%\textbf{X} & \textbf{R} & \textbf{Segment length} \\ \hline
%4          & 2          & 32000                   \\ \hline
%4          & 3          & 32000                   \\ \hline
%4          & 4          & 32000                   \\ \hline
%6          & 2          & 32000                   \\ \hline
...
\begin{figure}[H]
    ...
    \caption{\label{graf:somelossTODO2}Hodnota loss při trénování modelů s~různou velikostí hyperparametrů}
\end{figure}
...
Pro vyhodnocení úspěšnosti a kvality modelů byly použity metriky sdr vyjadřující poměr XXX, pesq říkající YYY a stoi vyjadřující ZZZ.

\todo{miniÚvod, co bude zkoumano a vyhodnocovano.}

\todo{Ucelene: referovat tabulku s~Hyper parametry nahore.
Kazda podkatpiola bude mit hrafy jako sisnr vs X,R, pesq vs x a r, stoi vs x, r, jakoze v~podkapitole Vliv X budou grafy takove, ze pro kazdou metriku bude graf jeden, kde se meni X a je tam videt jak se jednotlive hodnoty metriky lisi pro ruzna X.
}
\todo{Mozna: Pametova narocnost modelu. Spektrum nahravek.}

\todo{rozdelit tabulku podle HzperParam a spis mensi tabulku ukazujici vysledku pro konkretni hyper parametry.
experimenty:
...
\subsection{Vliv hyper--parametrů X a R}
\todo{Pro obe podkapitoly X a R: zmensovani a zvetsovani X, vysledky testovani, nejake fajnove grafy porovnani vysledky mezi jednotlivymi X, obrazky, grafy, pohlavi histogramy}
...
%---------------------------------------------------------------------------------------------
\subsection{Vliv délky segmentů}
\todo{zmensovani a zvetsovani SL, vysledky testovani, nejake fajnove grafy porovnani vysledky mezi jednotlivymi SL, grafy, histogramy pohlavi}
...
\textbf{X} & \textbf{R} & \textbf{Délka segmentů} & \textbf{SDR {[}dB{]}} & \textbf{STOI} & \textbf{PESQ} \\ \hline
8 & 4 & 32000 & 12.452 & 0.934 & 2.693 \\ \hline
8 & 4 & 16000 & 12.494 & 0.934 & 2.697 \\ \hline
\end{tabular}
\end{table}
...
%---------------------------------------------------------------------------------------------
\subsection{Úspěšnost modelů s~ohledem na pohlaví mluvčích}
\todo{nebo zahrnout do podkapitol. histogram vysledku skrze jednotlive promluvy, zjistit pomery jeslti je lepsi vysledky muz zena nebo muz muz. Zkusit to treba pro nejlepsi model, pripadne pro nejakej jeden mensi.}
...
\todo{Shrnout jejich vysledky. Muj nej a srovnat s~tim jejich nejlepsim, pripadne takovej, kde jsou parametry podobne tem mejm treba.}
...
\todo{dopsat rozdil oproti te me a pripadne co muze byt pricina.}
...
\todo{shrnuti a porovnani jednotlivych zmen a vysledku mezi sebou komplexne. Pripadne porovnani vysledku s~ref studii, obrazky, grafy. Okomentovat, co je tedy dulezity parametr, a ktery jde zmensit a neni moc dulezity z~pohledu vysledku evaluace. okomentovat take pohlavi vysledky, prumery sdr, mean, a pod.}
\todo{Zda se, ze parametr XYZ nema na SDR takovy vliv, jako parametr ABC...}
